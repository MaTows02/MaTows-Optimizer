...
# 0. GESTION DES ERREURS (POUR NE PAS QUE CA SE FERME)
Trap {
...
# 2. INTERFACE XAML (CORRIGEE)
[xml]$xaml = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
        Title="MaTows Optimizer - V1" Height="950" Width="1280" 
...
function Add-Log($msg) {
    ...
    # Pause courte pour que l'oeil suive
...
$window.FindName("btnSelectAll").Add_Click({ $l = @("chkRestore","chkTemp","chkTelem","chkHibern","chkWifiSense","chkPrint","chkGameDVR","chkUltPerf","chkThrot","chkMouse","chkSticky","chkSysResp","chkIndex","chkDns","chkExt","chkBing","chkBgApps","chkCopilot","chkWidget"); foreach ($n in $l) { if ($isLaptop -and ($n -eq "chkUltPerf" -or $n -eq "chkHibern")) { continue }; $window.FindName($n).IsChecked = $true } })
$window.FindName("btnUnselectAll").Add_Click({ $l = @("chkRestore","chkTemp","chkTelem","chkHibern","chkWifiSense","chkPrint","chkGameDVR","chkUltPerf","chkThrot","chkMouse","chkSticky","chkSysResp","chkIndex","chkDns","chkExt","chkBing","chkBgApps","chkCopilot","chkWidget"); foreach ($n in $l) { $window.FindName($n).IsChecked = $false } })
...
$window.FindName("btnApply").Add_Click({
    ...
    if ($window.FindName("chkGameDVR").IsChecked) { Add-Log "Arret GameDVR..."; New-ItemProperty "HKCU:\System\GameConfigStore" -Name "GameDVR_Enabled" -Value 0 -Type DWord -Force -ErrorAction SilentlyContinue | Out-Null; Add-Log "OK." }
